...
class Market:
    ...
    def getHistorical1Day(self):
        ...
        url = f"https://api.tiingo.com/iex/{Market.market}/prices?startDate={startDate}" \
              f"&resampleFreq=8hour&afterHours=false&forceFill=true&token={Market.apikey}&endDate={endDate}"
        x = requests.get(url)
    ...
    def getLivePrice(self):
        ...
        url = f"https://api.tiingo.com/iex/?tickers={Market.market}&token={Market.apikey}"
        x = requests.get(url)
        df = pd.DataFrame(x.json())
        ...
        Market.data = Market.data.append(df, ignore_index=True)
    ...
    def getRSI1Day(self):
        ...
        if missing == 0:
            ...
            Market.RSI1Day = rsis.rsi().values[-1]